/*
  ...
  Allows input of lower/upper case keys, symbols (but not spaces) {EDIT: Spaces added by Joshua}.
...
#define RES_STRING_X        75
#define RES_STRING_Y        50

#define BG_GRID_COL DIM_GRAY
#define SEL_GRID_COL TAN
...
void draw_square(uint16_t x, uint16_t y, char data, uint16_t col) {
    ...
    } else if(data == 0x6) { //Space
        display_char_xy_col('S', x + SQUARE_OFFSET + 1, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('p', x + SQUARE_OFFSET + 6, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('a', x + SQUARE_OFFSET + 11, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('c', x + SQUARE_OFFSET + 16, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('e', x + SQUARE_OFFSET + 21, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
    } else if(data == 0x7) { //Switch upper/lower
        if(curr_array == lower_arr) {
...
//Differences are there is a quit button and no backspace button
void draw_square_mine(uint16_t x, uint16_t y, char data, uint16_t col) {
    //Fill background
    fill_rectangle_c(x + SQUARE_OFFSET, y + SQUARE_OFFSET, K_GRID_SIZE - SQUARE_OFFSET, K_GRID_SIZE - SQUARE_OFFSET, col);
    if(data == 0x6) { //Switch alphabet/symbols
        display_char_xy_col('Q', x + SQUARE_OFFSET + 1, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('u', x + SQUARE_OFFSET + 7, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('i', x + SQUARE_OFFSET + 13, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
        display_char_xy_col('t', x + SQUARE_OFFSET + 18, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
    } else if(data == 0x7) { //Switch upper/lower
        if(curr_array == lower_arr) {
            uint8_t off = TEXT_OFFSET_X(3 * (CHAR_WIDTH + 1)) + x + SQUARE_OFFSET;
            display_char_xy_col('A', off, y + SQUARE_OFFSET + TEXT_OFFSET_Y, WHITE, col);
    ...
    if(last >= K_ROWS * K_COLUMNS) {
        ...
        for(y = 0, i = 0; y < K_ROWS; y++, y_offset += K_GRID_SIZE) {
            for(x = 0, x_offset = START_X; x < K_COLUMNS; x++, i++, x_offset += K_GRID_SIZE) {
                if(i == selected)
                    draw_square(x_offset, y_offset, curr_array[i], SEL_GRID_COL);
                else
                    draw_square(x_offset, y_offset, curr_array[i], BG_GRID_COL);
            }
        }
        ...
        draw_square(x_offset, y_offset, curr_array[last], BG_GRID_COL);
        //draw selected
        x_offset = (selected % K_COLUMNS) * K_GRID_SIZE + START_X;
        y_offset = (selected / K_COLUMNS) * K_GRID_SIZE + START_Y;
        draw_square(x_offset, y_offset, curr_array[selected], SEL_GRID_COL);
    }
}

//Only differences are that is uses draw_square_mine()
void draw_grid_mine(uint8_t selected, uint8_t last) {
    uint8_t x, y, i;
    uint16_t x_offset, y_offset;
    if(last >= K_ROWS * K_COLUMNS) {
        y_offset = START_Y;
        for(y = 0, i = 0; y < K_ROWS; y++, y_offset += K_GRID_SIZE) {
            for(x = 0, x_offset = START_X; x < K_COLUMNS; x++, i++, x_offset += K_GRID_SIZE) {
                if(i == selected)
                    draw_square_mine(x_offset, y_offset, curr_array[i], SEL_GRID_COL);
                else
                    draw_square_mine(x_offset, y_offset, curr_array[i], BG_GRID_COL);
            }
        }
    } else if(last != selected) {
        //clear last
        x_offset = (last % K_COLUMNS) * K_GRID_SIZE + START_X;
        y_offset = (last / K_COLUMNS) * K_GRID_SIZE + START_Y;
        draw_square_mine(x_offset, y_offset, curr_array[last], BG_GRID_COL);
        //draw selected
        x_offset = (selected % K_COLUMNS) * K_GRID_SIZE + START_X;
        y_offset = (selected / K_COLUMNS) * K_GRID_SIZE + START_Y;
        draw_square_mine(x_offset, y_offset, curr_array[selected], SEL_GRID_COL);
    }
}
...
//Difference is no display of the k_str
void draw_keyboard_no_word() {
    if(sel != last_sel) {
        draw_grid_mine(sel, last_sel);
        last_sel = sel;
    }
    if(string_pos < last_string_pos) {
        fill_rectangle_c(RES_STRING_X,RES_STRING_Y,MAX_STRING_SIZE * (CHAR_WIDTH + 2),CHAR_HEIGHT, display.background);
...
/*
  ...
  =================================================================
  Adapted by Joshua Gregory 2019
*/
